...
uint64_t load_elf_pie(void *elf_data, uint64_t preferred_base)
{
	...
	for (int i = 0; i < header->e_phnum; i++) {
		Elf64_Phdr *phdr =
		    (Elf64_Phdr *)(phdr_table_addr + (i * header->e_phentsize));
	...
	for (int i = 0; i < header->e_phnum; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			uint64_t vaddr_start =
			    preferred_base + (phdr->p_vaddr - elf_base);
			uint64_t vaddr_end = vaddr_start + phdr->p_memsz;

			// Align to page boundaries for PMM
			uint64_t page_start =
			    vaddr_start & ~((uint64_t)PAGE_MASK);
			uint64_t page_end =
			    (vaddr_end + PAGE_MASK) & ~((uint64_t)PAGE_MASK);

			log(host, O_OKAY,
			    "PT_LOAD[%d]: VA=0x%p-0x%p (pages=0x%p-0x%p)\n", i,
			    vaddr_start, vaddr_end, page_start, page_end);
	...
	log(host, O_OKAY,
	    "ELF base=0x%p, e_entry=0x%p, offset=0x%p, final entry=0x%p\n",
