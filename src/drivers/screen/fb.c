...
void draw_char(char c, int x, int y, uint32_t fg) {
    ...
    for (int row = 0; row < 16; row++) {
        ...
        // Calculate the exact starting pixel in the framebuffer for this row
        ...
        if (data & 0x80) line[0] = fg;
        if (data & 0x40) line[1] = fg;
        if (data & 0x20) line[2] = fg;
        if (data & 0x10) line[3] = fg;
        if (data & 0x08) line[4] = fg;
        if (data & 0x04) line[5] = fg;
        if (data & 0x02) line[6] = fg;
        if (data & 0x01) line[7] = fg;
    }
}
#include <utils/misc/inline.h>
// Source - https://stackoverflow.com/a
// Posted by Sep Roland, modified by community. See post 'Timeline' for change history
...
void write_serial_n(uint64_t n, int base) {
    ...
    char *t = "0123456789ABCDEF";
    int i = 0;
    if (n == 0) { write_serial_string("0"); return; }
    while (n > 0) {
        buf[i++] = t[n % base];
        n /= base;
    }
...
void vkprintf(const char* fmt, va_list args) {
    for (const char* p = fmt; *p != '\0'; p++) {
        ...
        if (ty >= 720) scroll_up(); // your screen overflow check
        ...
        switch (*p) {
            ...
            case 'U': fg_color = 0xFF0000; break;
            case 'q': fg_color = 0x00FF00; break;
            case 'Q': fg_color = 0xFFFFFF; break;
            case 'r': fg_color = 0xFFFF00; break;
