...
#define host "com.strawberry.drivers.vfs"
/* --- Constants & Types --- */
...
struct cpio_header {
	...
	char devmajor[8], devminor[8], rdevmajor[8], rdevminor[8], namesize[8],
	    check[8];
...
int grow_file_table()
{
	...
	if (file_table) {
		...
		memcpy(new_table, file_table,
		       table_capacity * sizeof(vfs_node_t));
...
void vfs_mount()
{
	...
	while (curr + 110 <= vfs_end) {
		...
		if (memcmp(name_ptr, "TRAILER!!!", 10) == 0)
			break;

		memset(file_table[total_files].name, 0, 64);
		memcpy(file_table[total_files].name, name_ptr,
		       (n_size > 63) ? 63 : n_size);
	...
	log(host, O_OKAY, "VFS: Mounted %d files. Cap: %d\n", total_files,
	    table_capacity);
}
...
{
	log(host, O_OKAY, "MBI Pointer: %p", addr);
	...
	while (tag->type != 0) {
		if (tag->type == 3) {
			...
			// IF THESE ARE > 4GB (0x100000000), YOUR PAGING WILL
			// CRASH
			log(host, O_OKAY, "Module found: %p - %p\n",
			    mod->mod_start, mod->mod_end);

			if (mod->mod_start >= 0x100000000) {
				kprintf("CRITICAL ERROR: Module loaded above "
					"4GB limit!\n");
				for (;;)
					;
			}
...
int vfs_read(const char *filename, char *out_buffer, uint32_t max_len)
{
	for (int i = 0; i < total_files; i++) {
		if (file_table[i].used &&
		    path_match(file_table[i].name, filename)) {
			uint32_t to_copy = (file_table[i].size >= max_len)
					       ? max_len - 1
					       : file_table[i].size;
...
{
	log(host, O_OKAY, "Get file size of: %q OK %Q\n", filename);
	for (int i = 0; i < total_files; i++) {
		if (file_table[i].used &&
		    path_match(file_table[i].name, filename))
...
// out: array of char[64]
// max: max number of entries to fill
int vfs_list_dir(const char *path, char out[][64], int max)
{
	...
	uintptr_t curr = vfs_start;

	while (curr + sizeof(struct cpio_header) <= vfs_end && count < max) {
		struct cpio_header *h = (struct cpio_header *)curr;
		if (memcmp(h->magic, "070701", 6) != 0)
			break;
		...
		uint32_t n_size = hex_to_u32(h->namesize, 8);
		uint32_t f_size = hex_to_u32(h->filesize, 8);
		char *name_ptr = (char *)(curr + sizeof(struct cpio_header));

		if (memcmp(name_ptr, "TRAILER!!!", 10) == 0)
			break;

		// skip entries that don't start with the path
		if (strncmp(name_ptr, path, path_len) != 0) {
			uintptr_t next =
			    (uintptr_t)(name_ptr + n_size + f_size + 3) & ~3;
			curr = next;
			continue;
		}

		// point past the matched path part
		char *sub = name_ptr + path_len;
		if (*sub == '/')
			sub++;

		// grab first path component
		char buf[64];
		int j = 0;
		while (sub[j] && sub[j] != '/' && j < 63) {
			buf[j] = sub[j];
			j++;
		}
		buf[j] = '\0';
