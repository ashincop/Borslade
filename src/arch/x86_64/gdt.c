...
{
	// Ensure flags are in the high 4 bits and limit_high is in the low 4
	// bits
...
void split_byte_to_nibbles(uint8_t input, uint8_t *high_nibble,
			   uint8_t *low_nibble)
{
...
void gdt_set_tss_gate(int index, uint64_t base, uint32_t limit)
{
	...
	// Since this is 16 bytes, it will overwrite gdt[index] and
	// gdt[index+1].
...
// Create a separate stack for the kernel to use when we're interrupted in User
// Mode
...
void gdt_set_gate(uint8_t index, uint32_t base, uint32_t size,
