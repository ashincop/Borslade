...
void pmm_mark_used64(uint64_t addr) {
    if (PAGE_SIZE == 0 || total_pages == 0) {
        log(host, O_FATAL, "pmm_mark_used64 called before init (PAGE_SIZE=%d total_pages=%d)\n", (int)PAGE_SIZE, (int)total_pages);
        return;
    }
    if (!bitmap) {
    ...
    if (page >= total_pages) {
        log(host, O_FATAL, "pmm_mark_used64 out-of-range addr=%p page=%d total_pages=%d\n", (void*)addr, (int)page, (int)total_pages);
        return;
    }
    ...
    if (bit_index >= bitmap_size) {
        log(host, O_INFO, "pmm_mark_used64 BITMAP OOB: page=%d bit_index=%d bitmap_size=%d bitmap=%p addr=%p\n", (int)page, (int)bit_index, (int)bitmap_size, bitmap, (void*)addr);
        asm volatile("cli");
...
void* pmm_alloc_page() {
    ...
    uint64_t bitmap_size = (total_pages + 7) / 8;
    if (pmm_verbose) log(host, O_OKAY, "PMM: pmm_alloc_page total_pages=%d bitmap_size=%d bitmap=%p\n", (int)total_pages, (int)bitmap_size, bitmap);
    // Use precomputed reserved pages from init_alloc
    uint64_t reserved_pages = pmm_reserved_pages;
    if (pmm_verbose) log(host, O_OKAY, "PMM: reserved_pages=%d\n", (int)reserved_pages);
        ...
        if (bitmap[i] != 0xFF) {
            for (uint64_t j = 0; j < 8; j++) {
                ...
                if (!(bitmap[i] & (1ULL << j))) {
                    ...
                    if (pmm_verbose) log(host, O_OKAY, "PMM: pmm_alloc_page allocating page %d addr=%p\n", (int)page, (void*)addr);
                    pmm_mark_used64(addr);
                    return (void*)addr;
                }
            }
...
void* pmm_alloc_pages(uint64_t count) {
    ...
    if (pmm_verbose) log(host, O_OKAY, "PMM: pmm_alloc_pages count=%d total_pages=%d\n", (int)count, (int)total_pages);

    uint64_t max_start = total_pages - count; // inclusive
    uint64_t bitmap_size = (total_pages + 7) / 8;
    uint64_t reserved_pages = pmm_reserved_pages;
    if (pmm_verbose) log(host, O_OKAY, "PMM: pmm_alloc_pages reserved_pages=%d max_start=%d bitmap_size=%d\n", (int)reserved_pages, (int)max_start, (int)bitmap_size);
    for (uint64_t i = 0; i <= max_start; i++) {
        ...
        for (uint64_t j = 0; j < count; j++) {
            ...
            uint64_t bit_index = page / 8;
            if (bit_index >= bitmap_size) {
                if (pmm_verbose) kprintf("PANIC: pmm_alloc_pages BITMAP OOB access: page=%d bit_index=%d bitmap_size=%d\n",
        ...
        if (found) {
            ...
            if (pmm_verbose) log(host, O_OKAY, "PMM: pmm_alloc_pages found range start_page=%d addr=%p count=%d\n", (int)i, addr, (int)count);
...
void pmm_debug_range(uint64_t addr, uint64_t bytes) {
    if (!bitmap) {
        if (pmm_verbose) kprintf("[%q OK %Q] PMM_DEBUG: bitmap is NULL\n");
        return;
    }
    ...
    uint64_t bitmap_size = (total_pages + 7) / 8;
        ...
        if (pmm_verbose) log(host, O_OKAY, "PMM_DEBUG: addr=%p bytes=%d start_page=0x%x end_page=0x%x total_pages=0x%x bitmap=%p bitmap_size=%d\n", (void*)addr, (int)bytes, (int)start_page, (int)end_page, (int)total_pages, bitmap, (int)bitmap_size);

    if (end_page >= total_pages) if (pmm_verbose) kprintf("PMM_DEBUG: RANGE OUT OF RAM (end_page >= total_pages)\n");

    uint64_t bstart = (start_page / 8 >= 2) ? (start_page / 8 - 2) : 0;
    uint64_t bend = bstart + 8;
    if (pmm_verbose) log(host, O_OKAY, "PMM_DEBUG bitmap[%d..%d]:", (int)bstart, (int)bend);
    for (uint64_t i = bstart; i <= bend && i < bitmap_size; i++) {
        if (pmm_verbose) kprintf(" %x", (int)bitmap[i]);
    }
    if (pmm_verbose) kprintf("\n");

    // Dump first 32 bytes at addr (safe-ish; caller should ensure it's valid)
    uint8_t* p = (uint8_t*)addr;
    if (pmm_verbose) log(host, O_OKAY, "PMM_DEBUG bytes at addr %p:", (void*)addr);
    for (int i = 0; i < 32; i++) {
        if (pmm_verbose) kprintf(" %x", (int)p[i]);
    }
    if (pmm_verbose) kprintf("\n");
}
// --- Initialization ---
...
void init_alloc(uint64_t mem_size_in_bytes, uint64_t mb_addr) {
    ...
    log(host, O_OKAY, "PMM INIT: mem_bytes=%d PAGE_SIZE=%d total_pages=%d bitmap=%p bitmap_size=%d kernel_end=%p\n", (int)mem_size_in_bytes, (int)PAGE_SIZE, (int)total_pages, bitmap, (int)bitmap_size, (void*)kernel_end_ptr);
    ...
    log(host, O_OKAY, "PMM INIT: bitmap_page_start=%d bitmap_pages=%d pmm_reserved_pages(low)=%d reserved_end(low)=%p (guard=%u pages)\n", (int)bitmap_page_start, (int)bitmap_pages, (int)(pmm_reserved_pages), (void*)reserved_end, PMM_RESERVED_GUARD_PAGES);
