...
void task_b_main() {
    print("[%q OK %Q] LAUNCHD has been launched\n");
...
void init_multitasking() {
    ...
    *(uint64_t*)(frame_baseA +152) = 0x10;                        // SS = kernel
    ...
    kprintf("[%q OK %Q] multi: Kernel taskA ready at %p\n", frame_baseA);
    void* test = read("./a.out");
    uint8_t* check_test = (uint8_t*)test;
    // 4. Spawn launchd user task
    spawn_user_task((uint64_t)task_b_main, 1, "launchd.sys", 0);
    uint64_t test_size = 1024*8;
    uint64_t test_ptr = 0x8000000;
    ...
    uint64_t test_payload = test_ptr;
    uint64_t test_base = test_payload - sizeof(malloc_header_t);
    uint64_t test_entry = load_elf_pie(test, test_base);
    spawn_user_task(test_entry, 10, "test.sys", 0);
}
