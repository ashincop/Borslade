...
void *find_xsdp_from_mb2(uint64_t mbi_phys_addr)
{
	...
	struct mb2_info_header *mbi =
	    (struct mb2_info_header *)(uintptr_t)mbi_phys_addr;
	...
	while (tag->type != MB2_TAG_TYPE_END) {
		...
		if (tag->type == MB2_TAG_TYPE_ACPI_NEW) {
			// The RSDP structure starts right after the 8-byte tag
			// header
		...
		tag = (struct mb2_tag *)((next_addr + (MB2_TAG_ALIGN - 1)) &
					 ~(MB2_TAG_ALIGN - 1));

		// Safety check to prevent reading past the MBI total size
		if ((uintptr_t)tag >= (mbi_phys_addr + mbi->total_size))
			break;
	}
