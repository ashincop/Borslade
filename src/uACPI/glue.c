...
{
	return (uacpi_thread_id)1; // Constant ID since we use spinlocks for
				   // isolation
}
...
uacpi_status uacpi_kernel_schedule_work(uacpi_work_type t, uacpi_work_handler h,
					uacpi_handle ctx)
{
	(void)t;
	h(ctx);
...
#define IO_FUNC(name, type, inst)                                              \
	uacpi_status uacpi_kernel_io_##name(uacpi_handle h, uacpi_size o,      \
					    type *v)                           \
	{                                                                      \
...
{
	uint32_t bdf =
	    0x80000000 | (a.bus << 16) | (a.device << 11) | (a.function << 8);
	*o = (uacpi_handle)(uintptr_t)bdf;
	return UACPI_STATUS_OK;
}
...
{
	for (uint32_t i = 0; i < (uint32_t)usec * 1000; i++) {
		asm volatile("pause");
	}
}
...
typedef uint64_t uacpi_interrupt_state;
#include <stdbool.h>
uacpi_interrupt_state uacpi_kernel_disable_interrupts(void) {
    uacpi_interrupt_state rflags;
    asm volatile("pushfq; pop %0; cli" : "=rm"(rflags) : : "memory");
    return rflags;
}

void uacpi_kernel_restore_interrupts(uacpi_interrupt_state state) {
    asm volatile("push %0; popfq" : : "rm"(state) : "memory", "cc");
}
