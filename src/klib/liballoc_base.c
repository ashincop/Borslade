...
#define host "com.strawberry.kernel.allocation"
...
void pmm_mark_used64(uint64_t addr)
{
	if (PAGE_SIZE == 0 || total_pages == 0) {
		if (pmm_verbose)
			log(host, O_FATAL,
			    "pmm_mark_used64 called before init (PAGE_SIZE=%d "
			    "total_pages=%d)\n",
			    (int)PAGE_SIZE, (int)total_pages);
		return;
	}
	if (!bitmap) {
		kprintf("[%U ERROR %Q] pmm_mark_used64 bitmap is NULL\n");
		return;
	}
	...
	if (page >= total_pages) {
		if (pmm_verbose)
			log(host, O_FATAL,
			    "pmm_mark_used64 out-of-range addr=%p page=%d "
			    "total_pages=%d\n",
	...
	if (bit_index >= bitmap_size) {
		if (pmm_verbose)
			log(host, O_INFO,
			    "pmm_mark_used64 BITMAP OOB: page=%d bit_index=%d "
			    "bitmap_size=%d bitmap=%p addr=%p\n",
			    (int)page, (int)bit_index, (int)bitmap_size, bitmap,
			    (void *)addr);
		asm volatile("cli");
		for (;;)
			asm volatile("hlt");
	...
	if (pmm_verbose) {
		if (pmm_verbose)
			log(host, O_OKAY, "PMM: mark page %d (addr=%p)\n",
			    (int)page, (void *)addr);
...
void *pmm_alloc_page()
{
	...
	if (pmm_verbose)
		log(host, O_OKAY,
		    "PMM: pmm_alloc_page total_pages=%d bitmap_size=%d "
		    "bitmap=%p\n",
	...
	if (pmm_verbose)
		log(host, O_OKAY, "PMM: reserved_pages=%d\n",
		    (int)reserved_pages);
	for (uint64_t i = 0; i < bitmap_size; i++) {
		if (bitmap[i] != 0xFF) {
			for (uint64_t j = 0; j < 8; j++) {
				...
				if (page >= total_pages)
					break; // don't scan past end
				if (page < reserved_pages)
					continue; // skip reserved low pages
				if (!(bitmap[i] & (1ULL << j))) {
					uint64_t addr =
...
void *pmm_alloc_pages(uint64_t count)
{
	...
	if (pmm_verbose)
		log(host, O_OKAY,
		    "PMM: pmm_alloc_pages count=%d total_pages=%d\n",
	...
	if (pmm_verbose)
		log(host, O_OKAY,
		    "PMM: pmm_alloc_pages reserved_pages=%d max_start=%d "
		    "bitmap_size=%d\n",
	...
	for (uint64_t i = 0; i <= max_start; i++) {
		...
		for (uint64_t j = 0; j < count; j++) {
			...
			if (bit_index >= bitmap_size) {
				if (pmm_verbose)
					kprintf("PANIC: pmm_alloc_pages BITMAP "
						"OOB access: page=%d "
						"bit_index=%d bitmap_size=%d\n",
						(int)page, (int)bit_index,
						(int)bitmap_size);
				asm volatile("cli");
				for (;;)
					asm volatile("hlt");
		...
		if (found) {
			if (i < reserved_pages)
				continue; // don't hand out pages inside
					  // reserved region
			void *addr = (void *)(i * PAGE_SIZE);
			if (pmm_verbose)
				log(host, O_OKAY,
				    "PMM: pmm_alloc_pages found range "
				    "start_page=%d addr=%p count=%d\n",
				    (int)i, addr, (int)count);
			for (uint64_t j = 0; j < count; j++)
				pmm_mark_used64((uint64_t)addr +
						(j * PAGE_SIZE));
			return addr;
		}
	}
...
void pmm_debug_range(uint64_t addr, uint64_t bytes)
{
	if (!bitmap) {
		if (pmm_verbose)
			kprintf("[%q OK %Q] PMM_DEBUG: bitmap is NULL\n");
		return;
	}
	...
	if (pmm_verbose)
		log(host, O_OKAY,
		    "PMM_DEBUG: addr=%p bytes=%d start_page=0x%x end_page=0x%x "
		    "total_pages=0x%x bitmap=%p bitmap_size=%d\n",
		    (void *)addr, (int)bytes, (int)start_page, (int)end_page,
		    (int)total_pages, bitmap, (int)bitmap_size);

	if (end_page >= total_pages)
		if (pmm_verbose)
			kprintf("PMM_DEBUG: RANGE OUT OF RAM (end_page >= "
				"total_pages)\n");

	uint64_t bstart = (start_page / 8 >= 2) ? (start_page / 8 - 2) : 0;
	uint64_t bend = bstart + 8;
	if (pmm_verbose)
		log(host, O_OKAY, "PMM_DEBUG bitmap[%d..%d]:", (int)bstart,
		    (int)bend);
	...
	// Dump first 32 bytes at addr (safe-ish; caller should ensure it's
	// valid)
	uint8_t *p = (uint8_t *)addr;
	if (pmm_verbose)
		log(host, O_OKAY, "PMM_DEBUG bytes at addr %p:", (void *)addr);
	for (int i = 0; i < 32; i++) {
		if (pmm_verbose)
			kprintf(" %x", (int)p[i]);
	}
...
void init_alloc(uint64_t mem_size_in_bytes, uint64_t mb_addr)
{
	...
	log(host, O_OKAY,
	    "PMM INIT: mem_bytes=%d PAGE_SIZE=%d total_pages=%d bitmap=%p "
	    "bitmap_size=%d kernel_end=%p\n",
	    (int)mem_size_in_bytes, (int)PAGE_SIZE, (int)total_pages, bitmap,
	    (int)bitmap_size, (void *)kernel_end_ptr);

	// Compute reserved low pages used by kernel
	uint64_t kernel_reserved_pages =
	    (kernel_end_ptr + PAGE_SIZE - 1) / PAGE_SIZE;
	// Add a guard of extra reserved pages so kmalloc/pmm allocations won't
	// land immediately after the kernel (helps avoid accidental overlap)
	...
	log(host, O_OKAY,
	    "PMM INIT: bitmap_page_start=%d bitmap_pages=%d "
	    "pmm_reserved_pages(low)=%d reserved_end(low)=%p (guard=%u "
	    "pages)\n",
	    (int)bitmap_page_start, (int)bitmap_pages,
	    (int)(pmm_reserved_pages), (void *)reserved_end,
	    PMM_RESERVED_GUARD_PAGES);
	...
	for (uint64_t addr = mb_addr; addr < mb_addr + mb_info_size;
	     addr += PAGE_SIZE) {
		pmm_mark_used(addr);
	}
...
void split_block(malloc_header_t *block, uint64_t size)
{
	...
	// Only split if the leftover space is large enough to hold a header +
	// some data
	uint64_t min_split_size = sizeof(malloc_header_t) + 16;

	if (block->size > size + min_split_size) {
		malloc_header_t *new_block =
		    (malloc_header_t *)((uint8_t *)(block + 1) + size);
...
void *kmalloc(uint64_t size)
{
	...
	while (curr) {
		if (curr->magic != HEAP_MAGIC) {
			// If magic is wrong here, the heap was already
			// corrupted elsewhere
			asm volatile("sti");
			return NULL;
		}
	...
	// 4. INITIALIZE: Set up the new block header at the start of the new
	// pages
	malloc_header_t *new_block = (malloc_header_t *)new_mem;
	new_block->magic = HEAP_MAGIC;
	new_block->size =
	    (pages_to_alloc * PAGE_SIZE) - sizeof(malloc_header_t);
	new_block->is_free = 0;
	new_block->next = NULL;
	...
	while (curr && curr->next) {
		if (curr->is_free && curr->next->is_free) {
			...
			if (curr_end == (uint8_t *)curr->next) {
				curr->size +=
				    sizeof(malloc_header_t) + curr->next->size;
